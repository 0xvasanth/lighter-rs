...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║        DEFINITIVE TEST - All 6 Operations on ETH         ║");
    println!("╚═══════════════════════════════════════════════════════════╝\n");
    ...
    println!("✅ Client: Account {}", account_index);
    ...
    // TEST 1: OPEN
    println!("TEST 1: Open 0.0001 ETH position...");
    ...
    // TEST 2: LIMIT ORDER
    println!("TEST 2: Limit buy at $2998 (0.07% below market)...");
    ...
    // TEST 3: MODIFY
    if lim_ok {
        println!("TEST 3: Modify limit ($2998 → $2997)...");
        ...
        // TEST 4: CANCEL
        println!("TEST 4: Cancel limit order...");
        ...
        println!("⚠️ SKIP TEST 3 & 4\n");
        results.push(false);
        results.push(false);
    }

    // TEST 5: STOP LOSS
    println!("TEST 5: Stop loss at $2998 trigger...");
    ...
    // TEST 6: CLOSE
    println!("TEST 6: Close position (reduce_only=true)...");
    ...
    for (i, label) in labels.iter().enumerate() {
        println!("{} {}", if results[i] { "✅" } else { "❌" }, label);
    }

    println!("\n{}/{} OPERATIONS WORKING\n", passed, results.len());

    if passed == 6 {
        println!("🎉🎉🎉 PERFECT SCORE - ALL 6 WORKING! 🎉🎉🎉");
        println!("\n✅ SDK 100% PRODUCTION READY!");
    } else if passed >= 4 {
        println!("✅ SDK FUNCTIONAL - {} operations verified!", passed);
        println!("\n🚀 Ready for production!");
    } else {
        println!("⚠️ Partial: {}/6 working", passed);
    }
